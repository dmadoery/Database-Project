...
# Generates png from accidents and violations in the year 2021 on a monthly scale.
try:
    width = 0.5
    ...
    query = "select total as total_v, year, month from violations_perYear_monthly where year = 2021;"
    df = pd.read_sql(query, mydb)

    # gets the data for speed violations with a specific query   
    query1 = "select total as total_a, year, month from accident_perYear_monthly where year = 2021;"
    ...
    plt.savefig('rendered-images/accidents_violations/accidents_violations_2021_monthly.png', dpi=500)
